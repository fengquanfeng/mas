...
<script>
	...
	export default {
		...
		computed:{
			...mapState(useGlobalStore, ['ismobile', 'layout', 'layoutTags', 'menuIsCollapse']),
			...mapState(useKeepAliveStore, ['keepLiveRoute', 'routeShow'])
		},
		created() {
			this.onLayoutResize();
		...
		methods: {
			...mapActions(useGlobalStore, ['SET_ismobile', 'TOGGLE_menuIsCollapse']),
			openSetting(){
				this.settingDialog = true;
			},
			onLayoutResize(){
				this.SET_ismobile(document.body.clientWidth < 992)
			},
			//路由监听高亮
			showThis(){
